...
struct MCPToolAdapterTests {
    ...
    func testToolArgumentsGetString() {
    ...
    func testToolArgumentsGetNumber() {
        let args = ToolArguments(raw: [
            "int": 42,
            "double": 3.14,
            "string": "2.5",
    ...
    func testToolArgumentsGetStringArray() {
        let args = ToolArguments(raw: [
            "array": ["a", "b", "c"],
    ...
    func testToolResponseImage() {
        ...
        #expect(response.isError == false)
        #expect(response.content.count == 1)
        if case .image(let data, let mimeType, _) = response.content[0] {
