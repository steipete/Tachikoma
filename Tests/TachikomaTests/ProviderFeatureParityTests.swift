//
//  ProviderFeatureParityTests.swift
//  TachikomaTests
//
...
struct ProviderFeatureParityTests {
    ...
    func testProviderConfigurationDefaults() throws {
        let config = ProviderConfiguration()
        
        #expect(config.maxTokens == 4096)
        #expect(config.maxContextLength == 128000)
    ...
    func testProviderConfigurationPresets() throws {
        ...
        #expect(anthropic.maxContextLength == 200000)
        ...
        #expect(google.maxContextLength == 1048576) // 1M tokens
    ...
    func testProviderFeatureDetection() throws {
    ...
    func testSystemMessageTransformation() async throws {
        let provider = MockProvider(
            capabilities: ModelCapabilities(
    ...
    func testAlternatingRolesEnforcement() async throws {
        let provider = MockProvider(
            capabilities: ModelCapabilities(
                supportsVision: false,
                supportsTools: false,
    ...
    func testVisionInputValidation() async throws {
        let provider = MockProvider(
            capabilities: ModelCapabilities(
    ...
    func testToolLimitEnforcement() async throws {
        let provider = MockProvider(
            capabilities: ModelCapabilities(
                supportsVision: false,
                supportsTools: true,
    ...
    func testProviderFeatureFlags() throws {
    ...
    func testProviderAdapterWrapping() throws {
        let provider = MockProvider(
            capabilities: ModelCapabilities(
                supportsVision: true,
                supportsTools: true,
